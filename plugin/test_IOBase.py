...
class TestIOPassThrough(unittest.TestCase):
    ...
    def test_encrypt_str(self):
        ...
        self.assertEqual(test_string, iobase.IOPassThrough().encrypt(test_string))

    def test_encrypt_list(self):
        self.assertEqual(self.test_strings, iobase.IOPassThrough().encrypt([s for s in self.test_strings]))

    def test_decrypt_str(self):
        test_string = self.test_strings[0]
        self.assertEqual(test_string, iobase.IOPassThrough().decrypt(test_string))

    def test_decrypt_list(self):
        self.assertEqual(self.test_strings, iobase.IOPassThrough().decrypt([s for s in self.test_strings]))
